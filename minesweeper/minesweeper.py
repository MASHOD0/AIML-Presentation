...
class MinesweeperAI():
    ...
    def add_knowledge(self, cell, count):
        ...
        """
        # marking the cell move that has been made
        self.moves_made.add(cell)

        # marking the current cell as being safe
        self.mark_safe(cell)
        ...
        for a in range(max(0, i-1), min(i+2, self.height)):
            for b in range(max(0, j-1), min(j+2, self.width)):
                if (a, b) != (i, j):
                    neighbours.add((a, b))
        # add neighbours and value to sentence
        self.knowledge.append(Sentence(neighbours, count))

        # mark aditional cells as safe or mines
        self.update_self_sentence()
